...
pub fn line(renderer: &mut Renderer, mut x0: i32, mut y0: i32, mut x1: i32, mut y1: i32) {
    ...
    for x in x_pixel_1..x_pixel_2 {
        let (dot_x, dot_y) = match steep {
            true => (intersect_y.round() as i32, x),
            false => (x, intersect_y.round() as i32),
        };
...
pub fn draw_shape(renderer: &mut Renderer) -> () {
    ...
    for (i, _) in shape.iter().enumerate().step_by(2) {
        ...
        match (begin_x, begin_y, end_x, end_y) {
            (Some(a), Some(b), Some(c), Some(d)) => match renderer.shape_type {
                ShapeMode::POINTS => {
                    dot(renderer, *a, *b);
                }
                ShapeMode::LINES => {
            ...
            (Some(a), Some(b), None, None) => match renderer.shape_type {
                ShapeMode::POINTS => {
                    dot(renderer, *a, *b);
                }
                ShapeMode::LINES => {
